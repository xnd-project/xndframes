...
def test_dataframe_constructor():
    v = xf.XndframesArray(TEST_ARRAY)
    df = pd.DataFrame({"A": v})
    assert isinstance(df.dtypes["A"], xf.XndframesDtype)
    assert df.shape == (3, 1)
    str(df)


def test_dataframe_from_series_no_dict():
    s = pd.Series(xf.XndframesArray(TEST_ARRAY))
    result = pd.DataFrame(s)
    expected = pd.DataFrame({0: s})
    tm.assert_frame_equal(result, expected)

    s = pd.Series(xf.XndframesArray(TEST_ARRAY), name="A")
    result = pd.DataFrame(s)
...
def test_dataframe_from_series():
    ...
    assert isinstance(result.dtypes["A"], xf.XndframesDtype)


def test_getitem_scalar():
    sa = xf.XndframesArray(TEST_ARRAY)
    ser = pd.Series(sa)
    result = ser[1]
...
def test_getitem_slice():
    ser = pd.Series(xf.XndframesArray(TEST_ARRAY))
    result = ser[1:]
    expected = pd.Series(xf.XndframesArray(TEST_ARRAY[1:]), index=range(1, 3))
    tm.assert_series_equal(result, expected)
...
def test_getitem_iterable():
    ser = pd.Series(xf.XndframesArray(TEST_ARRAY))
    result = ser[[0, 1]]
    expected = pd.Series(xf.XndframesArray(TEST_ARRAY[0:2]), index=range(0, 2))
    tm.assert_series_equal(result, expected)
